...
export class OngletServiceComponent implements OnInit {
  ...
  createformulaire() {
    ...
    this.http.post("http://localhost:8080/formation/formulaire/service", body, {
      headers: new HttpHeaders({
        "Content-Type": "application/json",
      }),
    }).subscribe(response => {
      console.log("Formulaire envoyé ")
      this.message = "Message envoyé - Un conseiller vous recontactera au plus vite " + this.newFormulaire.title + " " + this.newFormulaire.nom;
    });
